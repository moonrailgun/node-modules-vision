/* @flow */
/* eslint quotes: 0 */
// Fork from https://github.com/yarnpkg/yarn/blob/master/src/lockfile/parse.js
...
function* tokenise(input: string): Iterator<Token> {
  ...
  function buildToken(type: Token['type'], value?: Token['value']): Token {
...
class Parser {
  ...
  onComment(token: Token) {
    ...
    if (typeof value !== 'string') {
      throw new Error('expected token value to be a string');
    }
  ...
  parse(indent: number = 0): Object {
    ...
    while (true) {
      ...
      if (propToken.type === TOKEN_TYPES.newline) {
        ...
        this.unexpected(`Unknown token: ${propToken}`);
      }
    }
...
export function parseYarnLockfileYaml(
  str: string,
